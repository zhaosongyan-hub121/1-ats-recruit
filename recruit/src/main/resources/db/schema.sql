...
-- 由 Spring Boot 自动执行（application.yml 中 spring.sql.init.mode=always）
-- 每次启动重建表并插入演示数据，方便开发演示
-- =========================================================
...
CREATE TABLE `sys_user` (
  ...
  `role`       VARCHAR(20)  NOT NULL DEFAULT 'CANDIDATE' COMMENT 'ADMIN/HR/CANDIDATE',
  `company`    VARCHAR(100)          DEFAULT NULL COMMENT '所属公司（HR用）',
...
CREATE TABLE `job_position` (
  ...
  `company_logo` VARCHAR(10)           DEFAULT NULL COMMENT '公司Logo文字(1-2字)',
  ...
  `category`     VARCHAR(20)  NOT NULL DEFAULT 'SOCIAL' COMMENT 'CAMPUS(校招)/SOCIAL(社招)/INTERN(实习)',
  `location`     VARCHAR(50)           DEFAULT NULL COMMENT '工作地点',
  `salary`       VARCHAR(30)           DEFAULT NULL COMMENT '薪资范围',
  `education`    VARCHAR(30)           DEFAULT NULL COMMENT '学历要求',
  `experience`   VARCHAR(30)           DEFAULT NULL COMMENT '经验要求',
  `publish_user_id` BIGINT             DEFAULT NULL COMMENT '发布人ID(HR)',
  `created_at`   DATETIME              DEFAULT CURRENT_TIMESTAMP,
  `updated_at`   DATETIME              DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted`      TINYINT      NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
...
CREATE TABLE `candidate` (
  `id`               BIGINT       NOT NULL AUTO_INCREMENT,
  `user_id`          BIGINT                DEFAULT NULL COMMENT '关联注册用户 id',
...
-- ----------------------------
-- 投递记录表（application 是 SQL 保留字，使用反引号）
-- ----------------------------
DROP TABLE IF EXISTS `application`;
CREATE TABLE `application` (
  `id`            BIGINT       NOT NULL AUTO_INCREMENT,
  `candidate_id`  BIGINT       NOT NULL,
  `user_id`       BIGINT                DEFAULT NULL COMMENT '投递人用户ID（CANDIDATE）',
  `position_id`   BIGINT       NOT NULL,
  `status`        VARCHAR(30)  NOT NULL DEFAULT 'SUBMITTED' COMMENT 'SUBMITTED=已投递/初筛中, SCREENING_PASS=初筛通过待面试, INTERVIEWING=面试中, INTERVIEWED=面试完成待结果, OFFER=已发Offer, ACCEPTED=已接受Offer/录用, REJECTED=已拒绝',
  `current_round` INT          NOT NULL DEFAULT 1 COMMENT '当前面试轮次',
  `cover_letter`  TEXT                  DEFAULT NULL COMMENT '求职信',
  `hr_remark`     TEXT                  DEFAULT NULL COMMENT 'HR备注',
  `status_log`    TEXT                  DEFAULT NULL COMMENT '状态变更日志JSON',
  `created_at`    DATETIME              DEFAULT CURRENT_TIMESTAMP,
  `updated_at`    DATETIME              DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted`       TINYINT      NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  ...
  KEY `idx_user` (`user_id`),
  KEY `idx_candidate_position` (`candidate_id`,`position_id`)
...
-- ----------------------------
-- 初始化账号（密码统一为 admin123，方便演示）
-- admin / admin123 (管理员)
-- hr_tencent / admin123 (腾讯HR - 张HR)
-- hr_ali / admin123 (阿里HR - 李HR)
-- hr_bytedance / admin123 (字节HR - 王HR)
-- hr_meituan / admin123 (美团HR - 赵HR)
-- hr_baidu / admin123 (百度HR - 孙HR)
-- hr_jd / admin123 (京东HR - 周HR)
-- 5个 CANDIDATE 角色账号
-- ----------------------------
...
-- 注意：上面所有账号密码都是 admin123 和 hr123456（BCrypt hash 统一为 admin123 的hash，仅作演示用）

-- ----------------------------
-- 演示数据：岗位（覆盖社招/校招/实习，多个公司）
-- ----------------------------
...
CREATE TABLE `screen_rule` (
  ...
  `match_mode`   VARCHAR(20)  NOT NULL DEFAULT 'ANY' COMMENT 'ANY=匹配任一即可/ALL=必须全匹配/MIN=最小值',
